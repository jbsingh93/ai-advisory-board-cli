...
- **Windows shim resolution**: npm-installed `claude` lands as `.cmd`/`.ps1`. `resolveWinCommand()` walks PATHEXT to find the real path so `spawn` doesn't need `shell: true` (which would trigger DEP0190).
