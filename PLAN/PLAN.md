...
Every Phase 2-5 chunk in `CHECKLIST.md` now carries its own `**UI**` subsection (the dashboard surface mirroring the CLI verbs) and its own `**Playwright MCP regression specs**` subsection (one `specs/<flow>.md` plan per UI flow). The structural rule:
...
- Phase 6.6 chunk 3 holds Phase 1 + cross-cutting specs only. Phase 2-5 specs live in their owning phase. As each phase ships, its specs land as files under `specs/`.
