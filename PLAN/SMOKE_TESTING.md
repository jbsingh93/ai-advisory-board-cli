...
| Touching `gui/` or `src/gui/server.ts` | **Playwright MCP smoke** (separate doc: `PLAN/PLAYWRIGHT_MCP.md`) |
| Refactor that doesn't change behavior | Typecheck + build is enough |
| Pure documentation edit (`PLAN/`, `CLAUDE.md`, `README.md`) | Typecheck + build (or nothing, if no `.ts` changed) |
...
- `PLAN/PLAYWRIGHT_MCP.md` — the UI-side equivalent of this doc. Every meaningful change to `gui/` or `src/gui/server.ts` gets a Playwright MCP smoke before being declared done.
- `PLAN/CHECKLIST.md` — live status. The "What's running right now" section reports verified-via-smoke flows.
