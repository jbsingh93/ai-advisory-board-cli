...
- [~] `src/core/discussion/business-context-agent.ts` — **SUPERSEDED by Phase 1.5 Knowledge Wiki.** The auto-extract-BusinessContext idea is replaced by the wiki's auto-ingest hook on discussion conclude. See `docs/development/KNOWLEDGE_WIKI.md` and Phase 1.5 below.
...
**Spec:** `docs/development/KNOWLEDGE_WIKI.md` (authoritative — read this first).
**Plan section:** `docs/development/PLAN.md` Part 7.
...
#### Members MCP regression specs (`docs/specs/`)

- [x] `docs/specs/members-enhance.md` — open edit modal → click "Enhance with AI" → choose type → wait for streamed completion → fields populated → save → reload → persisted
- [x] `docs/specs/members-tools-allowlist.md` — open member → add a tool chip → save → reopen → persisted → on disk `.claude/agents/<slug>.md` frontmatter `tools:` line reflects the change
- [x] `docs/specs/members-sync-agents.md` — click "Regenerate agent files" → toast confirmation → `.claude/agents/` directory listing matches active member count
...
#### Principles MCP regression specs (`docs/specs/`)

- [x] `docs/specs/principles-explore-wizard.md` — click "Explore" on a principle → step 1 renders → answer → step 2 references step 1 answer → ... → step 5 → "Save" persists wizard output back into principle body
- [x] `docs/specs/principles-seed-starters.md` — empty workspace → "Seed starters" button visible and enabled → click → 8 cards appear → button disabled on second visit
...
#### Decision Coach MCP regression specs (`docs/specs/`)

- [x] `docs/specs/coach-chat.md` — open Coach → send message → streaming response renders → send follow-up → context preserved (response references prior turn)
- [x] `docs/specs/coach-session-list.md` — multiple sessions exist → list shows all sorted by recency → click middle → resumes with full history → delete oldest → confirm → removed from list
...
### Playwright MCP regression specs (`docs/specs/`)

- [x] `docs/specs/sparring-anchor-deepdive.md` — open discussion → click "Spar" on response (round 2, turn 1) → panel opens with anchor banner → send a message → response streams → close panel → reopen → state preserved.
- [x] `docs/specs/sparring-inject-back.md` — sparring session → click "Inject insight back" → preview modal → confirm → main discussion shows a new bubble with `type: 'sparring_injection'` + full provenance fields.
- [x] `docs/specs/sparring-session-list.md` — discussion with 3 spar sessions → list shows all three sorted by recency → click middle session → resumes with full history.
...
- [x] `docs/specs/actions-kanban-dragdrop.md` — drag card pending → in-progress → reload → status persisted
- [x] `docs/specs/actions-add-edit.md` — open add modal → fill fields → submit → card appears in pending → click card → edit description → save → reload → persisted
- [x] `docs/specs/actions-extract-from-discussion.md` — concluded discussion → click "Extract actions" → candidate list renders → accept 2 reject 1 → kanban shows the 2 accepted

---

## Phase 5 — Skill creator (the killer feature) ✅

**Spec:** `docs/development/SKILL_CREATOR.md` (authoritative — read this first).
**Plan section:** `docs/development/PLAN.md` Part 6 (Action Board scope cut) with the 2026-05-20 redirect banner pointing back here.
...
- [x] **Live smoke (the big one):** stub-mode `actions solve d525be59 --no-planner --stub --yes` on external test folder completed in 315ms end-to-end — produced a valid SKILL.md at `.claude/skills/phase-5-smoke-action/` matching the deterministic `grantedTools` projection (`Read, Write, Glob, Grep`); `linkedSkill` populated on the action item; `actions runs show <run-id>` renders the embedded Planner proposal; `skills list` shows it at project scope; `skills uninstall` archives it to `.snapshots/skills/phase-5-smoke-action-<ts>/`.
- [x] **REAL end-to-end smoke (2026-05-21):** full advisory-board discussion → action extraction → real Planner → real skill-creator → install end-to-end in 11m 59s wall-clock on real Claude calls. Topic: "We want to ship a 3-minute YouTube intro video for our Q3 launch in two weeks." 3-member chat (Elon Musk + Julian Bent Singh + Alexandra Chen, CFA) with structured `actionSteps[]` per response → 29 candidates extracted via the structured-data fast path → user picked "Ship Q3 launch YouTube video distribution pipeline" (action `e013a5f0`, linked to discussion `8f6ac172`) → `aab actions solve e013a5f0 --yes` → Planner surfaced a **maximalist tier with 5 integrations across 3 invocation kinds**: 1× `bash-curl` (YouTube Data API v3 resumable upload with publishAt scheduling), 1× `bash-cmd` (npm + VS Code CLI + git for lite-youtube facade homepage swap), 3× `chrome-extension` (YouTube Studio post-config, Google Slides sales-deck embed, LinkedIn native cutdown) — exactly the "GUI-only destination" pattern Examples 5-7 of `docs/development/SKILL_CREATOR.md §6.5b` predicted → skill-creator authored a 175-line SKILL.md + 5 reference files (preflight checklist + 2 LinkedIn copy templates + 2 metadata JSON templates) → installed at `.claude/skills/ship-q3-launch-youtube-video-distribution-pipeline/` → `linkedSkill` populated on the action → `aab actions runs show a1236ee1` re-renders the full Planner proposal with verbatim curl snippet + concrete chrome-extension handoffInstructions. **3 production bugs caught + fixed via this real-Claude smoke that the stub-mode + Playwright MCP smokes had not exercised:** (a) resolver missed the `marketplaces/` layer of `/plugin install` (commit `c7df596`); (b) Windows `ENAMETOOLONG` on the ~24KB Planner prompt — fixed by piping long prompts via stdin to `claude -p` (no positional value); (c) schema over-strict on `tiers.<key>.name` (Opus emits human display labels like "End-to-end launch pipeline" not the literal enum) + integration synonym remap (title/label/displayName → name). **The depth-of-feature thesis is now proven on real Claude calls, not just stubs.**
...
- [x] Playwright MCP regression specs (all 8 shipped):
  - [x] `docs/specs/skill-plan-only.md` — Plan button → proposal modal → export-to-md
  - [x] `docs/specs/skill-planner-maximalist.md` — Recipe A/D → ≥3 integrations across ≥2 surfaces → toggle behavior
  - [x] `docs/specs/skill-planner-replan.md` — proposal modal → Re-plan → feedback ≥10 chars → new proposal mentions feedback keyword
  - [x] `docs/specs/skill-solve-happy-path.md` — full Plan → Accept → solve → install end-to-end with `linkedSkill` populated
  - [x] `docs/specs/skill-run-telemetry.md` — live WS streams `skill_run_tool_call` events → planner stream renders them
  - [x] `docs/specs/skill-install-conflict.md` — overwrite-archives + rename + abort variants
  - [x] `docs/specs/skill-runs-history.md` — list + show + export (with proposal.md inside the bundle)
  - [x] `docs/specs/skills-tab.md` — list + show + test (clipboard copy) + uninstall + restore
...
**Spec:** `docs/development/SKILL_CREATOR.md` §6.3 (rewritten 2026-05-21 with the wiki-as-brain framing).
...
- [x] Discussion: spar from the UI _— shipped in Phase 3 §UI (⚔ Spar button on every `messageBubble`)_
...
- [x] Decision Coach chat view _— shipped in Phase 2 §Decision Coach UI (🧠 Coach nav + chat view + session list)_
- [x] Sparring 1:1 chat view (anchored to a response) _— shipped in Phase 3 §UI (sparring modal + transcript + composer + inject)_
- [x] Skill-creator run-launch + telemetry + preflight-wizard UI _— shipped in Phase 5 Chunk 6 (Plan/Solve buttons, planner-progress-pane, proposal modal, Skills tab)_
- [x] Per-member color from the agent file's `color:` frontmatter — `readMemberAgentColor()` in `src/agents/emit-member-agent.ts` parses the YAML frontmatter; `enrichMembers()` in `src/gui/server.ts` surfaces it on the wire so existing `m.color || colorForMember(m.name)` fallback in `gui/app.js` now resolves to the real frontmatter value; deterministic hash remains as the fallback when the agent file is absent or hand-edited away. 8 tests in `src/agents/__tests__/read-member-agent-color.test.ts`. **Live MCP smoke verified** on `smoke-kw-2026-05-19`: `Elon Musk → pink`, `Julian Bent Singh → yellow`, `Alexandra Chen, CFA → red` (all from the on-disk agent files, not the hash).
- [x] Token-usage / cost dashboard view — new `📊 Usage` nav tab; `GET /api/usage[?since=YYYY-MM-DD&limit=N]` REST endpoint aggregates JSONL logs via a pure `summariseUsage()` helper in `src/core/tokens/usage-summary.ts`; dashboard shows totals card row (cost / calls / total tokens / cached read), daily-spend sparkline (last 7/30/90/all), per-feature + per-model tables sorted by cost desc with inline cost-share bars. 8 tests in `src/core/tokens/__tests__/usage-summary.test.ts`. **Live MCP smoke verified** on `smoke-kw-2026-05-19`: totals `3 calls / 42.3k tokens / 16.9k cached`, by-feature `discussions 2 · sparring 1`, by-model `sonnet 2 · opus 1` (data sourced from the workspace's `token-usage/2026-05-19.jsonl`).
- [x] Light theme + theme toggle — `:root[data-theme="light"]` token overrides (light bg, darker member palette so contrast on white works); toggle button in sidebar footer persists choice to `localStorage` under `aab-theme`; `initTheme()` runs before bootstrap so there's no FOUC. Toggle has `data-testid="theme-toggle"`. **Live MCP smoke verified**: toggle cycles `<html data-theme>` between `light` (`--bg: #f8fafc`, `--text: #1a1f29`) and `dark`; choice persists across page reload via `localStorage["aab-theme"]`.
- [x] Mobile responsive sidebar — `@media (max-width: 760px)` collapses the grid to a single column, parks the sidebar at `translateX(-100%)`, exposes a floating hamburger button (`data-testid="sidebar-toggle"`); tapping opens the sidebar + scrim; tapping the scrim or any nav item closes it via `closeSidebar()` which is also called from `navigate()` so the menu auto-dismisses on routing. Above 761px the hamburger and scrim are force-hidden with `!important` so the desktop layout is unaffected. **Live MCP smoke verified** at 400×800: hamburger paints, click opens sidebar + scrim, clicking a nav item routes correctly and auto-closes the sidebar; resize back to 1280×800 restores static desktop layout (no transform, hamburger force-hidden). Live-smoke catch: an initial `hidden` attribute on the toggle button blocked the mobile breakpoint from painting it — fixed by dropping the attribute.

---

## Phase 6.6 — UI E2E tests with Playwright MCP ✅

**Spec:** `docs/development/PLAYWRIGHT_MCP.md` (authoritative — read this first).
**Plan section:** `docs/development/PLAN.md` Part 8.
...
- [x] `docs/development/PLAYWRIGHT_MCP.md` reference doc (setup, tool cheat sheet, snapshot vs vision, prompt patterns, MCP vs `@playwright/test`, troubleshooting, Windows notes, security)
- [x] `docs/development/PLAN.md` Part 8 — high-level rationale + phasing + acceptance
- [x] CHECKLIST entry (this section)

### Chunk 2 — Repo-wide UI hygiene (prereq for stable tests) ✅

- [x] Add `test-artifacts/` to `.gitignore` (already in `.gitignore`)
- [x] Add the `data-testid` registry entries from `PLAYWRIGHT_MCP.md` §6 to existing `gui/index.html` and `gui/app.js`-rendered elements: sidebar tabs (`tab-{route}` for all 9), new-discussion modal (`new-discussion`, `new-discussion-modal`, `new-discussion-question`, `new-discussion-member-<slug>`, `new-discussion-start`), discussion-row (`discussion-row-<shortId>`), chat-view continue / follow-up / HITL controls (`discussion-continue`, `discussion-followup-open`, `discussion-followup-input`, `discussion-followup-send`, `hitl-prompt`, `hitl-panel`, `hitl-option-<index>`, `hitl-reply-input`, `hitl-reply-submit`), typing bubbles (`member-typing-<slug>`), message bubbles (`member-message-<slug>-<turn>`), orchestrator decision card (`orchestrator-decision-<round>`), conclusion marker (`discussion-concluded`). `memberSlug()` helper added to `gui/app.js` mirroring `memberAgentSlug()`.
- [x] Accessibility pass: sidebar `<aside aria-label="Navigation sidebar">`, `<main role="main">`, `<nav aria-label="Main">`; every interactive element has visible label or `aria-label` (member chips → `<button role="checkbox" aria-checked aria-label="Toggle <name>">`; theme toggle has `aria-label="Switch to {next} theme"`; HITL textarea has `aria-label="Reply to the board"`); decorative icons → `aria-hidden="true"` (all `.nav-icon` emoji spans, `brand-mark`, `status-dot`, all avatar emoji + initials); live regions on typing-dots (`role="status" aria-live="polite" aria-label="<member> is thinking"`) and orchestrator decisions (`role="status" aria-live="polite"`); HITL panel → `role="dialog" aria-modal="true" aria-labelledby="hitl-reply-heading"`; chat-stream → `role="log" aria-live="polite" aria-relevant="additions"`; new-discussion / edit / confirm modals → `role="dialog" aria-modal="true"`.
- [x] Verified live: Playwright MCP `browser_navigate http://localhost:3737` + `browser_snapshot` + `browser_evaluate` query confirmed every required `data-testid` resolves; the snapshot shows the `complementary "Navigation sidebar"` and `navigation "Main"` landmarks landing correctly, all nav buttons labelled, `status [ref=…]: connected` confirms the ws-label live region, and the existing HITL discussion renders `hitl-prompt` + `hitl-panel` (dialog) + per-turn `member-message-elon-musk-<turn>` + per-round `orchestrator-decision-<round>` with `data-action="request_user_input"`.

### Chunk 3 — Smoke flow specs (markdown plans, not yet test code) ✅

Each spec lives under `docs/specs/<flow>.md` and is generated by driving the dashboard via Playwright MCP. Format: numbered steps + expected observations + `data-testid` references.
...
- [x] `docs/specs/discussion-happy-path.md` — new-discussion modal → start with 3 members → wait for round 1 → continue → conclude. No HITL. Spec covers `tab-discussions`, `new-discussion-*`, `discussion-row-<shortId>`, `chat-stream`, `member-typing-<slug>`, `member-message-<slug>-<turn>`, `orchestrator-decision-<round>`, `discussion-continue`, `discussion-concluded`.
- [x] `docs/specs/discussion-hitl.md` — start → orchestrator gates → HITL panel appears with options → respond with `--option 1` equivalent → round 2 runs. Covers `hitl-prompt`, `hitl-panel`, `hitl-option-<index>`, `hitl-reply-input`, `hitl-reply-submit`.
- [x] `docs/specs/discussion-follow-up.md` — follow-up `all` / `specific` / `subset` variants via member-chip selector. Three variants documented end-to-end with the `Follow-up · targeted` / `Follow-up · subset` label assertions.
- [x] `docs/specs/members-tab.md` — list / show / activate / deactivate (basic CRUD UI is already shipped in Phase 6.5).
- [x] `docs/specs/principles-tab.md` — equivalent CRUD coverage (basic CRUD UI is already shipped in Phase 6.5).
- [x] `docs/specs/knowledge-tab.md` — Phase 1.5 Knowledge tab: page list, page detail with `[[slug]]` resolution, ingest + query + lint panels, slug-map invalidation on `wiki_*` WS events.
- [x] `docs/specs/a11y-audit.md` — per-tab snapshot, list of unlabeled elements, top 3 fixes. Initial 2026-05-21 audit block already populated; Pattern D prompt included for re-runs.

### Chunk 4 — Regression repro library (one entry per CHANGELOG bug fix) ✅

- [x] `docs/specs/regressions/silent-empty-modal.md` — 0 active members → new-discussion modal shows the loud empty-state, NOT silently empty.
- [x] `docs/specs/regressions/hitl-after-maxturns.md` — discussion concludes via `maxTurns` while orchestrator wanted user input → `pendingUserRequest` cleared, no HITL UI visible after `discussion-concluded` marker.
- [x] `docs/specs/regressions/follow-up-strict-failure.md` — when one targeted member fails, no partial round is committed (verify by reloading the discussion and confirming round count unchanged).
- [x] Pattern: every fix in `CHANGELOG.md` from this date forward adds an entry here. (Codified at the top of the regressions folder — applies to all future bug-fix commits.)

### Chunk 5 — `@playwright/test` deterministic suite ✅

- [x] `npm install --save-dev @playwright/test` — `^1.49.0` pinned in `devDependencies`.
- [x] `playwright.config.ts` at repo root (projects: chromium default, firefox + webkit CI-only; trace `retain-on-failure`; screenshot `only-on-failure`; video `retain-on-failure`; list reporter local + html reporter in CI; `testIdAttribute: 'data-testid'`; `webServer` boots `tests/fixtures/start-ui-server.mjs`).
- [x] `tests/e2e/` directory + first `.spec.ts` generated from `docs/specs/discussion-happy-path.md` via the Pattern C prompt in `PLAYWRIGHT_MCP.md` §7. `tests/e2e/discussion-happy-path.spec.ts` ships 6 tests across Dashboard scaffolding, Members tab, Theme + sidebar a11y. **All 6 pass live (`AAB_UI_BASE_URL=http://127.0.0.1:3737 AAB_UI_SKIP_SERVER=1 npx playwright test --project=chromium` — 6 passed in 3.3s).**
- [x] `tests/fixtures/seeded-workspace.ts` — boots a tempdir workspace via `aab init --non-interactive --home --name e2e-<rand>`. Returns `{ workspacePath, projectRoot, env, mockStateFile }`. Sets `HOME`/`USERPROFILE`/`APPDATA` to a tempdir so the test never touches the real `~/.aabcli/`.
- [x] `tests/fixtures/mock-claude.ts` — type-only contract; the actual stub binary is `tests/fixtures/mock-claude.mjs` with cross-platform `bin/claude.cmd` + `bin/claude` shims. Supports `happy-path`, `request-user-input`, `conclude-immediately`, `one-member-fails` profiles via `AAB_MOCK_CLAUDE_PROFILE`. Persisted call counter via `AAB_MOCK_CLAUDE_STATE_FILE`.
- [x] `npm run test:e2e` script + `test:e2e:ui` for the Playwright UI mode (also `test:e2e:install` to install browsers).

### Chunk 6 — CI (Phase 6.6 → 6 boundary) ✅

- [x] GitHub Actions workflow: `.github/workflows/ui-e2e.yml` — matrix on Node 20+22, ubuntu-latest + windows-latest + macos-latest, 4-way shard.
- [x] `npx playwright install --with-deps` step.
- [x] `npm run test:e2e -- --shard=${{ matrix.shard }}/4` for parallelism.
- [x] Failure artifacts (trace + screenshot + video) uploaded as GitHub Actions artifacts (`playwright-report-*` always; `playwright-traces-*` on failure).
- [x] Optional: `expect(page).toHaveScreenshot()` visual regression baselines — left **deliberately deferred** until the dashboard's chrome stabilises further (noted in the workflow as an extension point; the `--update-snapshots` gating policy is documented in `CONTRIBUTING.md`).

### Cross-cutting (this phase) ✅

- [x] `.claude/settings.json` permission deny on `mcp__playwright__browser_run_code_unsafe` (RCE-equivalent — see `PLAYWRIGHT_MCP.md` §12). Committed at repo root.
- [x] `aab doctor` adds a check: warn if `.mcp.json` exists but `node_modules/@playwright/mcp/cli.js` doesn't (teammate forgot `npm install`). Live-verified via `aab doctor --agents-dir <project-root>` → `✓ Playwright MCP install — @playwright/mcp installed`.
- [x] `aab doctor` adds a check: warn if `node_modules/@playwright/mcp` exists but the platform-appropriate `ms-playwright` cache (`~/.cache/ms-playwright/`, `~/Library/Caches/ms-playwright/`, or `%LOCALAPPDATA%\ms-playwright\`) is empty (teammate forgot `npx playwright install`). Live-verified → `✓ Playwright browsers — cached at C:\Users\julia\AppData\Local\ms-playwright`.
- [x] `CONTRIBUTING.md` sections: (a) "UI changes require a Playwright MCP smoke run; reference at `docs/development/PLAYWRIGHT_MCP.md`", (b) "CLI changes require a live smoke from the external test folder; reference at `docs/development/SMOKE_TESTING.md`". Also includes setup, commit format, and prompt-hardening guardrails.
...
## Phase 7 — Boards (member groups) + Dynamic member targeting ✅

> **Authoritative spec:** `docs/development/SPEC_BOARDS_AND_DYNAMIC_MEMBERS.md`. PLAN.md Part 10 holds the rationale + locked design. This phase is mostly *finishing wiring that already exists*: the `Board` type, `boards.json`, and `loadBoards/saveBoard/updateBoard/deleteBoard` already ship (`src/storage/types.ts:26`, `fs-storage-service.ts:110`, `paths.ts:153`), `Discussion` already has `boardId/boardName`, and `addFollowUpQuestion` already does `all/specific/subset`. The genuinely new work is the board surface area, the orphan-prune, the participant snapshot, and mid-discussion member join with explicit catch-up.
>
> **Sub-feature 1 (User request #1):** create separate boards that group members (sage-council parity).
> **Sub-feature 2 (User request #2):** in a follow-up, select which members answer **and add a new member into a live discussion**.
>
> Each chunk below ships 4 layers where applicable (engine + CLI + GUI + MCP specs + tests) per the per-phase UI + MCP gate. Every user-visible chunk = one `changeset`.

### Chunk 1 — Boards storage finish + CLI (no UI) ✅

- [x] Extend `Board` type: add `slug: string` (unique, kebab from name) + optional `archivedAt` (`src/storage/types.ts:26-33`)
- [x] `saveBoard`/`updateBoard`: validate name (1–100) + description (0–500) + slug uniqueness (suffix `-2`/`-3` on collision) + memberIds dedup + each id references an existing member; snapshot `boards.json` to `.snapshots/` before overwrite (parity with members.json). *(Storage permits empty `memberIds` so a cascade-prune can empty a board; the ≥1 rule is enforced at the command/REST layer via `validateBoardFields`.)*
- [x] `pruneMemberFromBoards(storage, memberId)` helper → strip id from every board's `memberIds`, persist changed boards, return `{ affected, emptied }`; called from `deleteBoardMember` (CLI `members delete` + web `DELETE /api/members/:id`)
- [x] `resolveBoardMembers(storage, settings, opts)` with precedence: `--members` > `--board` > `AAB_BOARD` env > `settings.activeBoardId` > all-active (`src/core/boards/resolve-board-members.ts`)
- [x] `AppSettings.activeBoardId?: string` + `DEFAULT_SETTINGS` (undefined) + documented in `settings` command help
- [x] New `aab board` command (`src/commands/board.ts`, registered in `src/cli.ts`): `list`, `show`, `create`, `edit`, `add-member`, `remove-member`, `set-members`, `rename`, `delete`, `use <slug|->`, `current`
- [x] `resolveBoardToken` (name/slug/id/prefix) mirroring `resolveMemberToken` (`discuss.ts`, now exported)
- [x] Interactive multi-select for `create`/`set-members` via `askMultiSelect` when TTY + no `--members`; comma-list otherwise
- [x] `aab discuss start "<q>" --board <slug|id|name>` (mutually exclusive with `--members`); sets `discussion.boardId` + `boardName` snapshot + `selectedMemberIds`
- [x] Enforce `settings.maxMembersPerDiscussion` at discussion start — **block** with hint when a board exceeds the cap (open question #1)
- [x] `init` seeds a starter board **"Full Board"** (all seeded members) + sets it active (`src/commands/init.ts`)
- [x] Always echo the resolved panel in `discuss start` output ("Convening: Go-to-Market Board — …")
- [x] **Tests:** `resolve-board-members` precedence table; `pruneMemberFromBoards` (0/1/many boards, board emptied); slug uniqueness + validation bounds *(33 new unit tests across `board-helpers.test.ts`, `resolve-board-members.test.ts`, `prune-member-from-boards.test.ts`; full suite 308/308 passing)*
- [x] **Live CLI smoke** (external test folder, `smoke-boards-2026-06-03`): `board create`→`list`→`use`→`use -`→`current`→`show`→`add-member`/`set-members`/`rename`; `discuss start --board solo` convened exactly Elon + echoed "Convening: Solo — Elon Musk" + persisted `boardId`/`boardName`/`participants`; cap test blocked Full Board (3) at max=2; `members delete "Julian Bent Singh"` pruned 2 boards + warned

### Chunk 2 — Boards web (REST + GUI) ✅

- [x] REST (mirror member endpoints): `GET/POST /api/boards`, `PATCH/DELETE /api/boards/:id`, `POST /api/boards/:id/activate`, `GET /api/boards/active` (`src/gui/server.ts`) + boards added to `/api/state`
- [x] `DELETE /api/members/:id` returns `affectedBoards` + `emptiedBoards` (from `pruneMemberFromBoards`) for a UI toast (now 200 JSON instead of 204)
- [x] WS event family `board_created|board_updated|board_deleted|board_activated` (client refreshes + re-renders Members view)
- [x] **UI:** Boards management section inside the Members view (`renderMembersView`) — `BoardGroupCard`-style cards (overlapped avatars + `+N` overflow + member count + `★ active` badge + Set-active/Edit/Delete) + Create/Edit modal reusing `#edit-modal` shell with name + description + ordered member chip multi-select (`openBoardEditModal`)
- [x] **UI:** board picker on the new-discussion screen — board pills above the member chips; selecting a board pre-fills its members (editable; manual edits clear the board) + passes `{ boardId }` to `POST /api/discussions`
- [x] `data-testid` registry additions: `boards-section`, `board-card`, `board-card-title`, `board-create-btn`, `board-name-input`, `board-desc-input`, `board-member-chip` (`board-member-grid`), `board-save-btn`, `board-edit-btn`, `board-use-btn`, `board-delete-btn`, `new-discussion-board-tab`, `new-discussion-board-card`
- [x] **Playwright MCP regression specs:** `docs/specs/boards-manage.md`, `docs/specs/boards-start-discussion.md`
- [x] **Playwright MCP smoke** of both flows (verified live on `smoke-boards-2026-06-03`): Boards section rendered 5 cards w/ avatars + active badge; created "UI Smoke Board" (2 members) via the modal; "Set active" moved the `★` badge; new-discussion board picker rendered 5 pills, selecting "Solo" pre-filled members to Elon only; board REST + validation (empty/dup name → 400) curl-verified

### Chunk 3 — Discussion participant snapshot (robustness; pre-req for Chunk 4) ✅

- [x] `DiscussionParticipant { memberId, name, slug, title, joinedAtRound, catchUpMode?, removedAtRound? }` + `Discussion.participants?` (append-only) (`src/storage/types.ts`)
- [x] `ConversationRound.addedMemberIds?: string[]` for join-timeline rendering
- [x] Read-time normalizer: legacy discussions without `participants` synthesize from `selectedMemberIds` (or all-active responses) with `joinedAtRound: 1`; persist on next save (non-destructive) — `src/core/discussion/participants.ts` (`synthesizeParticipants`/`ensureParticipants`), called from `continueDiscussion` + `addFollowUpQuestion`
- [x] New discussions populate `participants` from founding members; keep `selectedMemberIds` in sync (`startDiscussion`)
- [x] Render layers read names/titles from the **snapshot**: CLI renderers already use each `Response.memberName` (a per-response snapshot, even more robust than `participants`); join events render member names from `discussion.participants` (`render-discussion.ts`, `render-discussion-markdown.ts`)
- [x] **Tests:** participant-snapshot normalizer legacy→synthesized + deleted-member fallback + idempotency (6 tests in `participants.test.ts`)
- [x] **Live CLI smoke:** stripped `participants` from a real discussion (`7acf417d`) → follow-up re-rendered correctly + back-filled Elon at `joinedAtRound: 1` and persisted the synthesized form

### Chunk 4 — Dynamic member add (engine + CLI) ✅

- [x] `AddFollowUpQuestionOptions.addMemberIds?: string[]` + `catchUpMode?: 'full'|'summary'|'fresh'` (`conversation-flow.ts`)
- [x] **Relax the candidate-pool restriction**: pool computed from the *updated* roster (`rosterIds ∪ newcomerIds`) so newly-added ids are eligible (`conversation-flow.ts`)
- [x] Validate `addMemberIds`: active member, exists, not already a participant; ensure `.claude/agents/<slug>.md` exists (emit if missing)
- [x] Catch-up message builder per mode (`src/core/discussion/catch-up.ts` + `build-user-message.ts`): `full` (existing path), `summary` (use `discussion.summary` or one `summarizeDiscussion` call rendered into a context block), `fresh` (original question + current follow-up only, empty history) — newcomers only; founding/returning members always get full context; summary degrades to full if no summary could be produced
- [x] Append to `participants` (with `joinedAtRound` + `catchUpMode`) + `selectedMemberIds` **in-memory**; persist only on full success (strict-abort rollback preserved — mutations happen after the strict member loop)
- [x] Keep orchestrator view **global** even when responses are scoped — orchestrator still runs over the full `candidatePool` + whole discussion (anti-pattern guard FM-2.4/2.5; unchanged)
- [x] Edge case: added-but-not-targeted member joins roster for future rounds, doesn't respond/catch-up this round (`respondingNewcomerIds` gating; covered by unit test)
- [x] Record `round.addedMemberIds`
- [x] CLI: `aab discuss follow-up <id> "<q>" [--add-member <m>]… [--add-members a,b,c] [--catch-up full|summary|fresh]`; `--add-member` resolves against **all active members**; soft hint when `--member <new>` targets someone not in the discussion
- [x] Telemetry: log `summary` catch-up call via the summarizer's token log (`operationType: 'catchup_summary'`)
- [x] **Tests:** `addMemberIds` pool expansion; strict-abort rolls back the participant addition; catch-up mode selection; already-participant/inactive rejection; not-targeted-newcomer roster join (6 tests `add-follow-up-member.test.ts` + 7 `catch-up.test.ts`; full suite 327/327)
- [x] **Live CLI smoke** (`smoke-boards-2026-06-03`): `follow-up 7acf417d … --add-member "Alexandra Chen, CFA" --catch-up summary` → both Elon (returning, full) + Alexandra (newcomer, summary) responded; `discuss show` rendered "+ joined this round: Alexandra Chen, CFA via summary"; persisted participant `joinedAtRound: 2`/`catchUpMode: summary`, `round.addedMemberIds`, generated summary, and a `catchup_summary` token-usage log; soft-hint + already-in-discussion + bad-mode guards all verified

### Chunk 5 — Dynamic member add (web) ✅

- [x] Extend `POST /api/discussions/:id/follow-up` body: `addMemberIds?: string[]`, `catchUpMode?` (`server.ts`); validate (active/exists/not-already-participant) + expand pool to existing + newcomers; pass `members: activeAll`
- [x] WS event `member_joined` (id, name, joinedAtRound, catchUpMode) broadcast from the new round's snapshot → live "X joined the discussion"
- [x] **UI:** "+ Add member" affordance in the follow-up composer (`renderFollowUpComposer`) listing active members not already present; selecting reveals a catch-up selector ("Full transcript · Summary · Fresh", default Full); newly-added chips visually distinct (`chip-added`, dashed border, `＋` suffix)
- [x] **UI:** "+N joined" affordance (`round-joined-badge`) on the round divider when `round.addedMemberIds` is non-empty, reading names/modes from `discussion.participants`
- [x] `data-testid`: `followup-add-member-btn`, `followup-add-member-list`, `followup-add-member-option`, `followup-catchup-select`, `round-joined-badge`
- [x] **Playwright MCP regression spec:** `docs/specs/follow-up-add-member.md`
- [x] **Playwright MCP smoke** (verified live): opened the round-2 discussion → `round-joined-badge` "＋1 joined" rendered (tooltip "Alexandra Chen, CFA (summary)"); follow-up composer "+ Add member" → revealed Priya Patel option → clicking added a dashed `PP Priya Patel ＋` chip + surfaced the Full/Summary/Fresh catch-up selector

### Open product questions (resolved per spec §6 defaults)

- [x] #1 Board exceeds per-discussion cap → **block** (implemented; CLI + web both 400/error with hint)
- [x] #2 Default catch-up mode → **full** (implemented; `summary` degrades to full if no summary producible)
...
- [ ] `src/core/prompts/wiki-merge.ts` → `buildUserFactMergePrompt(input)` — frames input as pure first-person user voice; mandates reading `wiki/index.md` slug-map + `Grep`/`Read` of candidate pages **before** writing; per-fact create/update/skip; **merge, don't append-dump**; conflict → newer-wins + `^[ambiguous]` + bump `updated:`; respect `userEdited: true` (→ `skipped`); reuse `skill-ingest.ts` frontmatter/`[[wikilink]]`/secrets contracts verbatim; **empty result is valid** (no mandatory source page)
- [ ] `src/core/knowledge/ingest-user-facts.ts` → `ingestUserFacts({ text, kind, discussionId?, sparringSessionId?, workspace, settings, storage? })` — mirrors `runIngestCore` plumbing (dirs → agent → parse → slug-map rebuild → manifest append) but: **no mandatory `wiki/sources/*.md`**, update-biased, `maxTurns ~8`, haiku (`ingestModel`)
- [ ] `ManifestSourceType` gains `'user-input'` (`src/core/knowledge/manifest.ts:14`)
...
- [ ] `src/core/knowledge/ingest-queue.ts` — per-workspace, **drains one ingest at a time** (prevents concurrent ingests racing the `wiki/index.md` slug-map rebuild + `.manifest.json` append → lost pages), enqueue returns immediately (callers never await)
- [ ] Debounce/coalesce a burst of utterances into **one** merge pass over concatenated text (cuts cost; more reconciliation context at once)
- [ ] Idempotency guard against the **same event** firing twice (double-click/retried POST) — keyed on event identity (`UserResponse.id` / `discussionId+round+kind`), **not** content similarity; never blocks a genuine re-mention (optional — omit if in doubt)
- [ ] Decide CLI short-process drain strategy (`closeContext` best-effort drain vs. next-run flush vs. detached drainer) — web server is long-lived, unaffected (open Q `USER_INPUT_INGEST.md` §7.3)

### Wire the four input points (`conversation-flow.ts` + `sparring-service.ts`)

- [ ] Initial question → enqueue `{ kind: 'initial_question' }` after `initialUserResponse` push + round-1 save (`conversation-flow.ts:~138/241`)
- [ ] Follow-up question → enqueue `{ kind: 'follow_up' }` after follow-up commit + save (`conversation-flow.ts:~1002/1052`)
- [ ] HITL response → **replace** `maybeAutoIngestUserResponse` with queue enqueue `{ kind: 'hitl_response' }`; drop the 40-char gate (`conversation-flow.ts:655`)
- [ ] Sparring message → enqueue `{ kind: 'sparring_message' }` after user msg persisted in `sendSparringMessage` (`src/core/sparring/sparring-service.ts` — currently **zero** wiki integration)
- [ ] All gated behind `autoIngestUserInputs` + existing `knowledgeWiki.enabled` + wiki-dirs-exist guards; fire **after** `saveDiscussion`/`saveSparringSession`

### Reconcile conclude-time transcript ingest

- [ ] Re-scope `ingestDiscussionRaw` (conclude hook at `conversation-flow.ts:244/589/1055`) to **advisor synthesis / consensus / decisions** (driven by `discussion.summary`) so it no longer double-processes the user's raw words (manifest hash dedup can't catch the transcript-vs-utterance overlap — different hash)

### Settings

- [ ] `autoIngestUserInputs: boolean` (default `true`) in `KnowledgeWikiSettings` (`src/storage/types.ts:403`) + `DEFAULT_KNOWLEDGE_WIKI_SETTINGS` (`:420`); document in `KNOWLEDGE_WIKI.md` §23
- [ ] Decide fate of `autoIngestUserResponses` — subsume into `autoIngestUserInputs` (HITL = one `kind`) vs keep independent (open Q `USER_INPUT_INGEST.md` §7.4)

### Tests + live smoke (mandatory — LLM-behavioural)

- [ ] Unit (vitest): queue serialization/debounce/coalesce, idempotency guard, settings gate, `wiki-merge` prompt snapshot
- [ ] **Live smoke** from external test folder (never project root): (1) mention "Acme sells to SMB retailers" → entity page created; (2) re-mention with no new info → **no new page / no redundant edit**; (3) re-mention with new info ("moved Acme upmarket to APAC enterprise") → **existing page updated, `updated:` bumped, no `acme-2.md`**; (4) sparring message with new fact → ingests (sparring wired); (5) `userEdited: true` page re-mentioned → **skipped**; (6) concurrency: fire several utterances fast → slug-map + manifest consistent (no lost pages)
- [ ] Docs: extend `KNOWLEDGE_WIKI.md` §16 (auto-ingest hook) to reference the user-fact merge path
...
- [x] Playwright MCP installed at project scope (`.mcp.json` + `@playwright/mcp@0.0.75` devDep) — every meaningful UI change must be exercised via MCP before being declared done (see Phase 6.6 + `docs/development/PLAYWRIGHT_MCP.md`)
- [x] **Per-phase UI + MCP gate (added 2026-05-19; enforced 2026-05-21):** any CLI feature in Phase 2-5 with a UI surface must list its UI scope **and** its `docs/specs/<flow>.md` MCP plan inside the owning phase, not deferred to Phase 6.5/6.6. Phase 6.5 is now polish + cross-cutting only; Phase 6.6 chunk 3 holds Phase 1 + cross-cutting specs only — Phase 2-5 specs live in their owning phase's `**Playwright MCP regression specs**` subsection. (Locked in by `CONTRIBUTING.md` two-track test gates and `docs/development/PLAYWRIGHT_MCP.md` rule 7 — non-negotiable for future PRs.)
...
- **Phase 6.6 (Playwright MCP UI tests):** ✅ closed (2026-05-21). All 6 chunks + cross-cutting shipped. Chunk 2: `data-testid` registry (`tab-<route>`, `new-discussion-*`, `discussion-row-<shortId>`, `chat-stream`, `member-typing-<slug>`, `member-message-<slug>-<turn>`, `orchestrator-decision-<round>`, `discussion-continue`, `discussion-followup-*`, `hitl-prompt`, `hitl-panel`, `hitl-option-<index>`, `hitl-reply-{input,submit}`, `discussion-concluded`) + a11y pass (`role=dialog aria-modal=true` on every modal + HITL panel, `role=status aria-live=polite` on typing/orchestrator/ws-status, `role=log aria-live=polite` on chat-stream, `aria-label` on every interactive element, `aria-hidden=true` on decorative icons, `<aside aria-label>` + `<nav aria-label="Main">` + `<main role="main">`). Chunk 3-4: 7 smoke specs (`discussion-happy-path`, `discussion-hitl`, `discussion-follow-up`, `members-tab`, `principles-tab`, `knowledge-tab`, `a11y-audit`) + 3 regression specs (`silent-empty-modal`, `hitl-after-maxturns`, `follow-up-strict-failure`). Chunk 5: `@playwright/test@^1.49` devDep + `playwright.config.ts` + `tests/fixtures/{seeded-workspace.ts, mock-claude.ts, mock-claude.mjs, start-ui-server.mjs}` + cross-platform `tests/fixtures/bin/claude{.cmd,}` shims + `tests/e2e/discussion-happy-path.spec.ts` (6 tests, all green live). Chunk 6: `.github/workflows/ui-e2e.yml` (Node 20+22 × ubuntu/macos/windows × shard 1-4 with trace+screenshot+video artifact upload). Cross-cutting: `.claude/settings.json` denies `mcp__playwright__browser_run_code_unsafe`, `aab doctor` gains `Playwright MCP install` + `Playwright browsers` checks (both verified ✓ live), `CONTRIBUTING.md` ships the two-track test-gate sections. Live MCP smoke confirmed every required testid resolves and the dashboard's a11y landmarks land correctly.
...
- **Phase 5 (Skill creator — the killer feature):** ✅ closed (2026-05-21). All 6 chunks shipped; spec at `docs/development/SKILL_CREATOR.md` is authoritative. **Engine:** 11 new files under `src/core/skill/` (`resolve-skill-creator.ts`, `recon/{pc-scan,wiki-recon,web-recon,web-probe,orchestrator}.ts`, `planner.ts`, `planner-review.ts`, `build-brief.ts`, `invoke-skill-creator.ts`, `adapter.ts`, `install.ts`, `persist-run.ts`, `solve-orchestrator.ts`) + the Planner system prompt at `src/core/prompts/skill-planner.ts` (the most important prompt in the CLI per the spec) + the `skillDesignProposalSchema` extension to `src/core/parsing/llm-response-schemas.ts` with `validateProposalSemantics` for hard gates that go beyond shape + `RESERVED_SKILL_NAMES`. **Runner:** `runClaude` grows `appendSystemPromptFile` + explicit `outputFormat` options. **CLI:** `aab actions plan|solve|runs {list,show,export,delete}` + new top-level `aab skills {list,show,test,uninstall,restore}` + `aab init --install-skill-creator` + `aab doctor` skill-creator/PC-scan/web-reachability checks. **GUI:** Plan + Solve buttons on every action card; 4-phase Planner progress pane with live tool-call stream; interactive proposal modal (tier radio + per-integration toggles + per-stakeholder toggles + narrative editor + export-md); Re-plan feedback modal (server-enforced ≥10 chars + max-3 cap); new Skills tab in sidebar. **Server:** `/api/actions/:id/plan`, `/api/plans/:planId`, `/api/plans/:planId/replan`, `/api/actions/:id/solve`, `/api/actions/:id/runs`, `/api/skill-runs/:id`, `/api/recon/environment`, `/api/skills`, `/api/skills/:name` + the full `planner_*` and `skill_run_*` WS event family. **Tests:** 80 new vitest unit tests across `resolve-skill-creator.test.ts` (12) + `pc-scan.test.ts` (6) + `wiki-recon.test.ts` (4) + `web-recon.test.ts` (9) + `planner.test.ts` (25) + `build-brief.test.ts` (5) + `adapter.test.ts` (9) + `install.test.ts` (3) + `solve-orchestrator.test.ts` (7 end-to-end with stub skill-creator, covering happy path + plan-only + no-install + missing-skill-creator failure). Full suite at 236/236 passing. **Specs:** all 8 Playwright MCP regression specs shipped under `docs/specs/skill-{plan-only,planner-maximalist,planner-replan,solve-happy-path,run-telemetry,install-conflict,runs-history,skills-tab}.md`. **Live smoke:** stub-mode end-to-end on the external test folder produced a valid SKILL.md at `.claude/skills/phase-5-smoke-action/` in 315ms — `linkedSkill` populated, `runs show` renders the embedded Planner proposal, `skills list` enumerates it, `skills uninstall` archives cleanly. Real-Claude smoke against §20a Recipes A/D/E/F deferred to user (each Planner run is ~$2.20). The depth-of-feature thesis — Planner reasons about ≥3 multi-tool orchestrations spanning ≥2 surfaces, including first-class `chrome-extension` and `computer-use` invocation kinds — is enforced at the schema + semantic-gate layer; the prompt embeds three condensed few-shot examples (Elgato + pricing + LinkedIn) that calibrate the reasoning.
- **Phase 7 (Boards + Dynamic member targeting):** ✅ closed (2026-06-03). All 5 chunks shipped 4-layer (engine + CLI + GUI + MCP specs + tests). **Boards:** `Board` gains `slug` + `archivedAt`; `saveBoard/updateBoard` validate + snapshot `boards.json`; `pruneMemberFromBoards` cascade-prunes on member delete (CLI + web); `resolveBoardMembers` precedence (`--members` > `--board` > `AAB_BOARD` > `settings.activeBoardId` > all-active); new `aab board {list,show,create,edit,add-member,remove-member,set-members,rename,delete,use,current}`; `discuss start --board` + per-discussion cap **block**; `init` seeds "Full Board" + active. **Web boards:** full `/api/boards` REST + `board_*` WS + Boards section in the Members view (cards w/ overlapped avatars + `★ active`) + create/edit modal + new-discussion board picker (pre-fills members). **Participant snapshot:** `Discussion.participants` (append-only) + `ConversationRound.addedMemberIds`; read-time normalizer back-fills legacy discussions (`participants.ts`); render reads the snapshot. **Dynamic add:** `addFollowUpQuestion` gains `addMemberIds` + `catchUpMode` (full/summary/fresh — `catch-up.ts`), relaxed candidate pool, strict-abort rollback of joins, orchestrator stays global, `catchup_summary` telemetry; CLI `follow-up --add-member/--add-members/--catch-up` + soft hint; web "+ Add member" affordance + catch-up selector + `member_joined` WS + `round-joined-badge`. **Tests:** 52 new unit tests (`board-helpers`, `resolve-board-members`, `prune-member-from-boards`, `participants`, `catch-up`, `add-follow-up-member`); full suite **327/327** passing. **Specs:** `docs/specs/boards-manage.md`, `boards-start-discussion.md`, `follow-up-add-member.md`. **Live smoke** (`smoke-boards-2026-06-03`): full board CLI lifecycle; `discuss start --board solo` echoed the panel + persisted `boardId`/`participants`; cap blocked Full Board at max=2; `members delete` pruned 2 boards; `follow-up --add-member "Alexandra Chen, CFA" --catch-up summary` ran a real 2-member round (returning + newcomer) + persisted the join + `catchup_summary` token log; legacy-stripped `participants` back-filled on follow-up. **Playwright MCP smoke**: Boards section + create modal + activate + new-discussion board picker pre-fill + follow-up "+ Add member" dashed chip + catch-up selector + `round-joined-badge` all verified.
- **Phase 6:** not started.
