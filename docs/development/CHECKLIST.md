...
- [x] Web UI: composer toggle "📚 Use Business Wiki" (gated on `exposeToCoach`), `PATCH /api/coach/sessions/:id`, `coach_session_updated` broadcast, `📚 wiki` badge on `usedWiki` turns
- [x] CLI: `aab coach --wiki` (new/resume) + `aab coach send … --wiki`; `UserError` hint when `exposeToCoach` is off
- [x] Verification: live CLI smoke (2026-06-18) — `--wiki` opt-in guard errors with hint; ON path used the wiki (Globex 55%-of-ARR fact → sharp **Embrace Reality** / **Be Direct and Honest** questions, no lecturing) with `usedWiki=true` on opener + send turns; write-side `coach_message` ingest fired + drained (raw capture + `wiki/log.md` line + 3 produced pages); OFF path unchanged (no `usedWiki`, no ingest). Unit test on `buildCoachWikiInstruction` guardrail (15 coach tests pass; 356 total). Live Playwright MCP smoke (2026-06-18) — `📚 Wiki: ON` toggle renders `[pressed]` in the composer (gated on `exposeToCoach`); both wiki-using assistant turns show the `📚 wiki` badge with tooltip; clicking the toggle flips it to OFF and `PATCH /api/coach/sessions/:id` persists `useBusinessWiki=false` to disk; coach markdown renders (headings/blockquotes/lists).
