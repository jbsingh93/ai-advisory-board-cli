...
##### Decision Coach × Business Wiki (shipped 2026-06-18 — spec: `docs/development/COACH_WIKI_CONTEXT.md`)

User-gated wiki context for the coach. Toggle ON = the wiki is wired to the coach **both ways** (reads business facts + the user's own ingested thoughts on-demand; ingests the user's coach messages back into the wiki). Toggle OFF = today's hermetic principles-mirror. Design law: **wiki is fuel for sharper principle-grounded questions, never subject matter the coach lectures on.** On-demand tool retrieval (not CLI pre-injection) — accepted cost tradeoff (quality > cost; do not optimize back). **The per-session toggle is the sole control — no global opt-in.** It's available directly in the New Coach Session modal and the composer; default OFF per session. (The original `knowledgeWiki.exposeToCoach` global gate was removed post-ship per user request — "toggleable directly, no settings step".)

- [x] `DecisionSession.useBusinessWiki?` + `DecisionMessage.usedWiki?` in `src/storage/types.ts` (no global setting — `exposeToCoach` removed)
- [x] `coachReply` (read side): when `useWiki`, extend `allowedTools` with `Read/Grep/Glob`, set `addDirs: [workspaceRoot]`, append `buildCoachWikiInstruction()` (catalog-first, "ask sharper questions, do NOT lecture", wiki > web layering); track tool-use via `onEvent` → `usedWiki`
- [x] Write side: new `UserInputKind: 'coach_message'` (+ `KIND_FRAMING`); fire gated fire-and-forget `ingestUserFacts` (via ingest-queue) after each user turn when `useBusinessWiki && autoIngestUserInputs`; `coachSessionId` provenance in `ingest-user-facts.ts`
- [x] Web UI: New Coach Session modal toggle (sets `useBusinessWiki` on create) + composer toggle "📚 Use Business Wiki" (always shown), `PATCH /api/coach/sessions/:id`, `coach_session_updated` broadcast, `📚 wiki` badge on `usedWiki` turns
- [x] CLI: `aab coach --wiki` (new/resume) + `aab coach send … --wiki` — sets `useBusinessWiki: true` (no opt-in to honor)
- [x] Verification: live CLI smoke (2026-06-18) — ON path used the wiki (Globex 55%-of-ARR fact → sharp **Embrace Reality** / **Be Direct and Honest** questions, no lecturing) with `usedWiki=true` on opener + send turns; write-side `coach_message` ingest fired + drained (raw capture + `wiki/log.md` line + 3 produced pages); OFF path unchanged (no `usedWiki`, no ingest). Unit test on `buildCoachWikiInstruction` guardrail (356 tests pass). Live Playwright MCP smoke — composer `📚 wiki` badge + toggle persistence verified. Re-smoke after the ungating change (2026-06-18) — the "📚 Use Business Wiki" toggle is present in the **New Coach Session modal** (no settings step); creating a session with it checked persists `useBusinessWiki=true`; the composer toggle is always shown and reflects state.
