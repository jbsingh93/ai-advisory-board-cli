# ai-advisory-board — AI Advisory Board CLI (Claude Code-native port)
...
### 2.4 Repository layout for `ai-advisory-board`
...
> **⚠ Superseded for BusinessContext (2026-05-10):** the flat-JSON `BusinessContext` / `BusinessContextAgent` plan below is replaced by the **Knowledge Wiki** (Karpathy-style LLM Wiki). See **Part 7** of this file and the dedicated spec at `docs/development/KNOWLEDGE_WIKI.md`. Sparring, principles, and the rest of §2.7 still apply as written.
...
> **⚠ Superseded for the CLI (2026-05-20):** the per-piece mapping table below (actionClassifier → direct call, taskOrchestratorAgent → direct call, skillBuilderAgent → port verbatim, …) was the right plan for a from-scratch reimplementation. We now delegate to Anthropic's `skill-creator` skill instead and have repointed Phase 5 around the **Skill Planner** (§6 of `SKILL_CREATOR.md`) as the headline value-add. Authoritative spec: **`docs/development/SKILL_CREATOR.md`**.
>
...
| Sage Council piece | ai-advisory-board implementation |
|---|---|
...
| Concern | Source (sage-council) | ai-advisory-board |
|---|---|---|
...
- **npm:** `npm i -g ai-advisory-board` — primary install path. Single binary `aab` on `$PATH`.
- **Homebrew (later):** `brew install ai-advisory-board` once we have a tap.
- **Single-file binary (later):** `pkg` or Node SEA build for offline installs.
- **Self-update:** `aab update` runs `npm i -g ai-advisory-board@latest` (with confirmation) and reports the diff.
...
> **⚠ Superseded for the CLI (2026-05-20):** the entire single-loop runtime described below — 7-tool virtual workspace, 60-turn cap, planner → tool turns → critic → repair → potency → security → trigger evaluator pipeline — is **not** ported. Anthropic's official `skill-creator` skill does this whole loop internally, with its own tuned implementation. The CLI invokes skill-creator headlessly via `claude -p --append-system-prompt-file` and captures the emitted package. Authoritative spec: **`docs/development/SKILL_CREATOR.md`** §8 (skill-creator invocation) + §19 Chunk 4 (the implementation chunk).
>
...
> **⚠ Superseded for the CLI (2026-05-20):** none of the four layout modes are ported. `skill-creator` emits its own layout (the single-loop-direct shape, roughly) directly into the run workspace; the CLI's adapter pass normalizes frontmatter and we install verbatim. Authoritative spec: **`docs/development/SKILL_CREATOR.md`** §9 (adapter) + §10 (install).
...
- **Repository:** monorepo not needed; single npm package `ai-advisory-board`.
...
- **Versioning:** semver. Major bumps for storage-schema breaks (with migrations). Minor bumps for new commands. Patch bumps for fixes. Changelog at `CHANGELOG.md`, named "**ai-advisory-board changelog**" (not "Sage Council") — inheriting the source's naming guardrail.
...
- **Pre-release:** every commit to `main` publishes `ai-advisory-board@next` automatically.
- **Self-update:** `aab update` invokes `npm i -g ai-advisory-board@latest`; reads `npm view ai-advisory-board version` to print the diff.
...
> **The full design spec lives in `docs/development/KNOWLEDGE_WIKI.md`.** That document is the authoritative source of truth for directory layout, frontmatter contracts, page-type taxonomy, naming rules, manifest format, the three workflows (ingest/query/lint), the auto-ingest hook, the CLI surface, the Web UI surface, the migration plan from `BusinessContext`, and the 8-chunk build phasing. This Part 7 is the high-level overview; for any detail not covered here, defer to `KNOWLEDGE_WIKI.md`.
...
- Write `docs/development/PLAYWRIGHT_MCP.md` reference (this part is just the index pointer).
...
- Smoke MCP-driven flows for: new-discussion modal, round-1 happy path, HITL respond, follow-up (specific / subset / all), continue-to-conclude. Each captured as a markdown test plan in `docs/specs/`.
- A11y audit per tab; backfill `aria-label` / `data-testid` gaps surfaced.

### Per-phase (Phase 2-5 features, each as its phase ships — added 2026-05-19)

Every Phase 2-5 chunk in `CHECKLIST.md` now carries its own `**UI**` subsection (the dashboard surface mirroring the CLI verbs) and its own `**Playwright MCP regression specs**` subsection (one `docs/specs/<flow>.md` plan per UI flow). The structural rule:

- Phase 6.5 is no longer the catch-all UI bucket — it's polish + cross-cutting + the shipped-views index. Feature-specific UI moves to the owning phase.
- Phase 6.6 chunk 3 holds Phase 1 + cross-cutting specs only. Phase 2-5 specs live in their owning phase. As each phase ships, its specs land as files under `docs/specs/`.
- The cross-cutting "Per-phase UI + MCP gate" line in `CHECKLIST.md` is the enforcement hook — any new CLI feature with a UI surface must list both deliverables in its owning phase before being declared done.

This closes the structural gap where Phase 2-5 read as CLI-only deliverables with all UI work deferred to a single late phase. The deferred-UI framing made sense when the dashboard didn't exist yet; it stops making sense now that Phase 1 + 1.5 have established the engine → CLI → UI → smoke-both pattern.
...
- `docs/development/PLAYWRIGHT_MCP.md` exists, documents the tool surface, the `data-testid` registry, the prompt patterns, the snapshot-vs-vision mode trade-off, the MCP-vs-`@playwright/test` split, Windows notes, and security.
- At least one canonical smoke flow (new-discussion → round 1 → continue → conclude) is recorded as a markdown spec in `docs/specs/discussion-happy-path.md`, written by driving the dashboard through Playwright MCP.
...
User decision (2026-05-19): every meaningful change to the **CLI** (`src/` excluding `src/gui/`) **must** be exercised via a live smoke against the real `aab` binary and real Claude calls before being declared done. Typecheck + build is necessary but not sufficient — the May 2026 Phase 1 closeout shipped past typecheck-only verification with two stacked Windows runner bugs that silently broke every multi-line LLM call. Both surfaced **only** when a live smoke was run; both are now permanently captured as reference regressions in `docs/development/SMOKE_TESTING.md` §9.

> **The full reference lives in `docs/development/SMOKE_TESTING.md`.** That document is the authoritative source for the test-folder convention, init bootstrap, the canonical smoke flow, PowerShell + Bash gotchas, verification signals, cleanup, and the reference-regression log. This Part 9 is the rationale + the locked decisions; for any detail not covered here, defer to `SMOKE_TESTING.md`.
...
- **Skip smoke**: pure docs (`docs/development/`, `CLAUDE.md`, `README.md`), comment-only edits, renames.
