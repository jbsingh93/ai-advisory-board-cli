# `specs/sparring-anchor-deepdive.md` — Sparring: anchor → deep dive → resume
...
2. Open a discussion that has at least one `messageBubble` (any member response card with `data-testid="response-card"`).
